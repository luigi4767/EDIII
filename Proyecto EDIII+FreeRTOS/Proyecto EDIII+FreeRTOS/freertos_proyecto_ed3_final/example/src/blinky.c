 ...
 *
 * Descripcion: Ponemos flags para controlar que pase los datos a la tarea de filtro.
 * */
...
void Configuracion_IO (void)
{
	...
	Chip_SCU_PinMux( GRUPO(2) , PIN(0) , SCU_MODE_INACT | SCU_MODE_INBUFF_EN | SCU_MODE_ZIF_DIS , SCU_MODE_FUNC4 );//rojo
	Chip_SCU_PinMux( GRUPO(2) , PIN(1) , SCU_MODE_INACT | SCU_MODE_INBUFF_EN | SCU_MODE_ZIF_DIS , SCU_MODE_FUNC4 );//verde
	Chip_SCU_PinMux( GRUPO(2) , PIN(2) , SCU_MODE_INACT | SCU_MODE_INBUFF_EN | SCU_MODE_ZIF_DIS , SCU_MODE_FUNC4 );//azul
	...
	Chip_SCU_PinMux( GRUPO(2) , PIN(10) , SCU_MODE_INACT | SCU_MODE_INBUFF_EN | SCU_MODE_ZIF_DIS , SCU_MODE_FUNC1 );// FAULT timer0 match2
...
SemaphoreHandle_t Semaphore_Send;
SemaphoreHandle_t Semaphore_Filter;
...
static void vSignalTask(void *pvParameters){
	...
	//SystemCoreClockUpdate();
	if( xSemaphoreTake(Semaphore_ADC,( TickType_t )portMAX_DELAY) == pdPASS ){


		Chip_UART_SendBlocking (LPC_USART2 , Mensaje, strlen(Mensaje)); //printf(Mensaje)


		while(contador1 < TEST_LENGTH_SAMPLES){
			...
			contador1++;
			Chip_GPIO_SetPinState(LPC_GPIO_PORT, PORT(PORT_LEDG), PIN(PIN_LEDG), (bool)true);
		}
		Chip_GPIO_SetPinState(LPC_GPIO_PORT, PORT(PORT_LEDG), PIN(PIN_LEDG), (bool)false);
		while(contador2 < TEST_LENGTH_SAMPLES){
	...
	//portTICK_RATE_MS
	if(xSemaphoreTake(Semaphore_Send,( TickType_t )portMAX_DELAY) == pdPASS ){

		for(ids=0;ids<TEST_LENGTH_SAMPLES;ids++){
			xQueueSend(Queue_Datos,&testInput_f32_1kHz_8kHz[ids],portMAX_DELAY);
		}
		flagSend=true;
	...
	xSemaphoreGive(Semaphore_Filter);
...
static void vFilterTask(){
	...
	while(1){
		...
		if(xSemaphoreTake(Semaphore_Filter, portMAX_DELAY) == pdPASS){

			for(idf=0;idf<TEST_LENGTH_SAMPLES;idf++){
				xQueueReceive(Queue_Datos,&testInput_f32_1kHz_8kHz[idf],portMAX_DELAY);
			}
			...
			** ------------------------------------------------------------------- */
			for(i=0; i < numBlocks; i++){
				...
				Chip_GPIO_SetPinState(LPC_GPIO_PORT, PORT(PORT_LED1), PIN(PIN_LED1), (bool)true);
			}
			...
			while(contador3 < TEST_LENGTH_SAMPLES){
			...
			Chip_GPIO_SetPinState(LPC_GPIO_PORT, PORT(PORT_LED3), PIN(PIN_LED3), (bool)true);
...
int32_t main(void){
	...
	Queue_Datos = xQueueCreate(TEST_LENGTH_SAMPLES , sizeof(float));


	vSemaphoreCreateBinary(Semaphore_ADC);

	vSemaphoreCreateBinary(Semaphore_Send);

	vSemaphoreCreateBinary(Semaphore_Filter);


	xSemaphoreTake(Semaphore_Send, portMAX_DELAY);
	xSemaphoreTake(Semaphore_Filter, portMAX_DELAY);
