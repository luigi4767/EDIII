/*
 * SE IMPLEMENTA UN FILTRO FIR CON CMSIS USANDO FREERTOS COLAS Y SEMAFOROS E INSTANCIAMOS LA MISMA TAREA
 */
...
int main(void)
{
	...
	xTaskCreate(Procesa_Datos, "Procesamiento", configMINIMAL_STACK_SIZE,
			(unsigned int) FILTRO_MANUAL , (tskIDLE_PRIORITY + 1UL), (TaskHandle_t *) NULL);

	/* Procesamiento de Datos */
	xTaskCreate(Procesa_Datos, "Procesamiento", configMINIMAL_STACK_SIZE,
			(unsigned int) FILTRO_DSP  , (tskIDLE_PRIORITY + 1UL), (TaskHandle_t *) NULL);
